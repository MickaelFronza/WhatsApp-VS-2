...
const handleChartbot = async (
  ...
  if (!isNil(queue) && !isNil(ticket.queueOptionId) && msg.body == "#") {
    // falar com atendente
    ...
    return;
  } else if (!isNil(queue) && !isNil(ticket.queueOptionId) && msg.body == "0") {
    ...
    if (count == 1) {
      ...
      option = await QueueOption.findOne({
        where: { option: msg.body, parentId: ticket.queueOptionId }
      });
    }
    if (option) {
